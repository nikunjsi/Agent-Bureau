...
| M5 — Workspace + git | Worktrees, leases, commits, integration branches | 🔶 In progress — part 1 done (2026-08-28): workspace registration, a real worktree created per employee at hire and removed at fire, the transactional lease guard proven under real concurrency (750 racing attempts, exactly one winner every time), task-assignment re-pointing from a real integration ref, and bidirectional startup reconciliation proven against real process kills at both crash windows — all seven part-1 gate items proven with real commands (`PROGRESS.md` 2026-08-28). **Not the milestone gate**: §28's actual M5 gate is "three simulated employees commit in parallel and merge cleanly; a deliberate conflict produces a blocker checkpoint" — commits, validators, and `--no-ff` merges don't exist yet, so that gate hasn't been attempted, let alone passed. |
...
| 7 | Two employees write the same file | 🔶 Designed-for, isolation only (M5 part 1) — one worktree per employee (a fully separate folder, not a shared checkout) means two employees can never race on the same file on disk. The risk this row actually names is the *merge-time* conflict once both sets of edits are combined, which is entirely unbuilt — no merge code exists yet (part 2) |
...
| 13 | User's own git client (terminal, GUI) runs concurrently against a repo Bureau is operating on | 🔶 Partially covered (M5 part 1) — `runGit()`'s per-repo queue only serializes Bureau's *own* git calls against each other; it cannot serialize against a genuinely separate process like the user's own terminal. It does retry (up to 4 attempts, short backoff) specifically on lock-contention failures (`index.lock`, "cannot lock ref"), so a transient collision with something like the user's own `git status` retries instead of failing the operation outright. Not exercised by a dedicated test yet — proving it against a real second, independently-spawned git process is still open |

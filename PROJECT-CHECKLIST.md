...
| M4 — Control channel + tool server | Agents can talk back to Bureau (nothing above this works without it) | **Session 1 of 2–3 done** — loopback HTTP server, per-employee tokens (real Windows ACL via `icacls`, proven by reading permissions back), the three endpoints (`/v1/policy/check`, `/v1/tool/:name`, `/v1/event`) sharing Zod schemas with the Core, the long-poll hold with all three named edge cases proven, and CLAUDE.md invariant #6 ("Core dies mid-hold → deny") proven against a real process kill. `bureau-hook`, `bureau-tools`, and MCP wiring are session 2. |

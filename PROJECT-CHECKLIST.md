...
| M6 — Permissions + budgets | Policy evaluator, budgets, circuit breaker, redactor | 🔶 In progress — session 1 of 3 done (2026-08-29): the real rule model/loader (7 immutable global denies verbatim, a real `role.tools_allow`/`tools_deny` tier, an explicit `additionalRules` seam for M7's pack loader), Windows path canonicalisation, the §11.3 evaluator (including the load-bearing `verdict === null` guard), tool-class mapping (`EngineCapabilities.toolClasses`/`networkTools`, all three adapters), effective autonomy computed per spawn (never persisted — the `autonomous`-requires-confirmation seam is real, migration 0004, no dialog yet), the loop detector. S1/S2/S3/S9/S10 green, each confirmed to fail when its guarded behaviour is broken. Sessions 2 (budgets, cost write path, rate limits) and 3 (circuit breaker, redactor, the full S1–S11 gate) remain. |
...
| 17 | Employee loops burning tokens | M6 | ✅ Mitigated (M6 session 1) — the loop detector (reuses `breaker.repeatedToolLimit`/`repeatedToolWindowS`, default 5 in 60s, exactly §11.3's own default) forces `ask` on the Nth identical `(tool, canonicalised-args)` call and emits `tool.loop_detected`. Downgrades an `allow` only — never touches an existing `deny`/`ask` |

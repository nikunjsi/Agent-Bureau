...
5. **Nothing outside the workspace is readable or writable**, at any autonomy level. Not overridable. **With one carve-out the mechanism has, which is why it is stated here rather than discovered:** `evaluator.ts` short-circuits every `bureau_`/`mcp__bureau__` tool to `allow` _before_ the seven immutable denies are scanned (§23.2 — "always allowed"). That is deliberate and correct, and it means **policy does not enforce this invariant for Bureau's own tools; the handler does.** Every Core-side path that touches the filesystem on an agent's behalf canonicalises and confines the target itself, fails closed, and is tested at the handler — never at the policy layer that never ran. `src/main/memory/memoryTarget.ts`, `src/main/chat/attachments.ts` and `src/main/controlChannel/toolHandlers/artifactPath.ts` (pre-M11 B-1: `bureau_task_done`'s artifact paths) are the three that exist; anything that joins them owes the same guard and the same test (AUDIT #10).

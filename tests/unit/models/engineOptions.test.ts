...
describe('engine_options schemas (§7.1.1/§6.5)', () => {
  it('claude-code options default mode to auto and accept an empty object', () => {
    ...
    expect(ClaudeCodeEngineOptionsSchema.parse({ mode: 'structured' })).toEqual({
      mode: 'structured',
    });
  });

  it('claude-code options reject mode:pty — structured-only (§7.7.1, M3 session 3)', () => {
    expect(() => ClaudeCodeEngineOptionsSchema.parse({ mode: 'pty' })).toThrow(
      /does not support mode:'pty'/,
    );
  });
  ...
  it('generic-pty options require command and ready_pattern', () => {
    ...
    const parsed = GenericPtyEngineOptionsSchema.parse({
      command: 'my-agent',
      ready_pattern: '^> $',
    });

...
describe('RoleSchema', () => {
  it('parses full_key and every JSON array column', () => {
    ...
    const row = {
      ...
      shared_prompts: JSON.stringify(['prompts/_shared/definition-of-done.md']),
      skills: JSON.stringify(['code', 'debug']),
      deliverable_types: JSON.stringify(['code']),
      input_types: JSON.stringify(['code']),
      ...
      escalate_when: JSON.stringify(['the same approach has failed twice']),
      reports: JSON.stringify({ on_complete: 'what changed', on_block: 'what you need' }),
      sprite_key: 'dev',
      role_options: '{}',
      engine_options: null,
      enabled: 1,

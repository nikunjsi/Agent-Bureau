...
import { registerProjectWorkspace, hireEmployeeWorktree, fireEmployeeWorktree } from '../../../src/main/workspace/employeeWorktree';
import { listWorktreesPorcelain, getCheckedOutBranch } from '../../../src/main/workspace/gitWorktree';
...
describe('hireEmployeeWorktree / fireEmployeeWorktree (§28 M5 items 1, 2, 7 — gate items 1 and 5)', () => {
  ...
  it('gate 1: hires 3 employees — worktrees on disk match `git worktree list --porcelain` AND the `worktrees` table; main tree checkout unchanged', async () => {
    ...
    const employees = [seedEmployee(db, { name: 'Ravi' }), seedEmployee(db, { name: 'Priya' }), seedEmployee(db, { name: 'Wei' })];
    const worktrees = [];
    for (const employee of employees) {
      worktrees.push(await hireEmployeeWorktree({ db, activityLog, project, employee, companyHomePath }));
    }

    // Real command, real output — exactly what the gate item asks for.
    const porcelainRaw = execFileSync('git', ['worktree', 'list', '--porcelain'], { cwd: repoPath, encoding: 'utf8' });
    ...
    expect(diskEntries, 'exactly 3 worktree entries — the main tree must not appear as a 4th').toHaveLength(3);
    ...
    for (const wt of worktrees) {
      ...
      expect(diskPathsReal.has(fs.realpathSync.native(wt.path)), `worktree ${wt.path} must be a real entry on disk`).toBe(true);
      ...
      const resolvedInMainRepo = execFileSync('git', ['rev-parse', project.base_ref], { cwd: repoPath, encoding: 'utf8' }).trim();
      expect(row!.base_commit).toBe(resolvedInMainRepo);
    }

    const branchAfter = await getCheckedOutBranch(repoPath);
    expect(branchAfter, "hiring employees must never change what's checked out in the main tree").toBe(branchBefore);
  });
  ...
  it('trap e: hiring "Ravi" then "ravi" collides loudly (WorktreeNameCollisionError), not silently sharing a directory', async () => {
    ...
    await expect(hireEmployeeWorktree({ db, activityLog, project, employee: raviLower, companyHomePath })).rejects.toThrow(
  ...
  it('gate 5: fires an employee — worktree removed & pruned, row deleted, worktree_id nulled, git.worktree_released emitted, branch retained', async () => {
    ...
    expect(getEmployeeById(db, employee.id)?.worktree_id, 'employees.worktree_id must be nulled').toBeNull();

    const branchListRaw = execFileSync('git', ['branch', '--list', branchName], { cwd: repoPath, encoding: 'utf8' });
     
    console.log('--- git branch --list (retained) ---\n' + branchListRaw);
    expect(branchListRaw.trim().length, `branch ${branchName} must be retained per §10.3/§10.6, not deleted by the fire flow`).toBeGreaterThan(
      0,
    );

    const events = db.prepare("SELECT * FROM events WHERE type = 'git.worktree_released'").all() as Array<{ employee_id: string | null }>;

# Progress archive — M0, M1, M1 audit+fixes, M2

Full, unabridged session-by-session detail for milestones M0 through M2,
archived out of the main `PROGRESS.md` at the M3->M4 boundary (2026-08-22)
to keep that file's per-session catch-up cost bounded. `PROGRESS.md` keeps
a one-paragraph summary of each milestone below plus every item that was
still open at archive time — nothing carried-forward was dropped, only
moved. This file is the complete original record, unedited.
...
- **#9 (SERIOUS) — raw SQL outside repositories**, in `reconcile.ts`,
  ...
  > **This record was true when written and is no longer.** The M0–M2
  > re-audit (2026-09-10) found #9 **regressed**: `reconcile.ts` had three
  > direct `UPDATE`s on `spend_usd_micros` / `lifetime_spend_usd_micros`
  > again, each on a column `repositories/usage.ts` already wrote — so
  > three columns had two owners, which is standing rule 6's shape and was
  > invisible to every test of either half. Re-fixed in fix session 2
  > (audit #4), this time with a guard:
  > `tests/unit/rawSqlWritesAreOwned.test.ts` fails on any write outside
  > `repositories/` that is not in an explicit, reasoned allowlist, and
  > pins `reconcile.ts` by name so a future allowlist entry cannot quietly
  > re-open it. A record claiming a fix that has since been undone is worse
  > than no record, which is why this note is here rather than only in the
  > newer document.

**#6 (SERIOUS) — not fixed this session.** See "What surprised me."

> **Built at pre-M11 P-10 (2026-09-17).** `tests/integration/job-object.test.ts` now has a grandchild case: the packaged app contains a dummy child, the dummy spawns a grandchild once contained, Bureau is killed by PID only, and both are gone. **The open question above was answered, and it was not the sandbox.** libuv puts every non-detached child process in its own kill-on-close job, so on Windows a non-detached Node or Electron child dies with its parent regardless of Bureau's Job Object. Measured three ways: a bare `node` parent in this tool's shell, the same through WMI (outside the tool's process tree), and the packaged app with `BUREAU_SMOKETEST_NO_CONTAIN=1`. All three reaped the orphans. That made both the original direct-child test and a naive grandchild test pass with containment switched off. The grandchild case therefore spawns both processes **detached**, which escapes libuv's job. Negative control: containment OFF, both alive 10 s after the kill; containment ON, both dead. The original direct-child case is still non-discriminating (plan §F), and `containProcess` has no production caller (plan §F, M11).

...
| N-17 | `attachments.ts` confinement has no junction or short-name test case | A "refuses a path that escapes only through a junction" test, mirroring `memoryWriteConfinement.test.ts` | DONE <pending>: two cases added to `attachmentConfinement.test.ts`: a junction inside the workspace pointing outside is refused, and a short-name spelling of an inside path is accepted against a long-form workspace (the case fails loudly on a machine with no short-name component instead of passing vacuously). The textual-normalisation mutation fails exactly these two |

...
| N-12 | Reconcile's standalone `git worktree prune` can be removed and the suite stays green | Reconcile test: delete a worktree directory externally, run `reconcile()`, assert `git worktree list --porcelain` no longer names it | DONE <pending>: `tests/integration/workspace/reconcilePrunesDeletedWorktree.test.ts`, real repo and worktree, real `reconcile()`. It asserts the path is listed before deletion and after the external delete, and gone after reconcile. Mutation (standalone prune removed) caught |

...
| P-11 | `.github/workflows/ci.yml` vs §11.7 "release-blocking" | CI runs lint, the spec checks, unit, package, integration, contract and e2e, but **never `npm run test:security`**. Most S-files run incidentally inside other suites, but the suite as a gate does not exist in CI | A `test:security` step in CI after integration, or proof that every file in the script already runs in CI plus a recorded decision. `securitySuiteCoverage` still green | DONE <pending>: a 'Security tests' step running npm run test:security after the integration step in .github/workflows/ci.yml. securitySuiteCoverage.test.ts gained a case asserting the step exists and follows integration; it failed before the step was added. securitySuiteCoverage still green (7/7). Whether CI itself goes green is verified on the next push, which is Nikunj's (E-6) |

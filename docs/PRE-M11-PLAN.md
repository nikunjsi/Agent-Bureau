...
| X-20 | §22.4, L3244 | No Settings entry: "Add a key for small helper tasks (optional — a few cents a month)" with the honest note | Settings offers the entry, the key is stored through the secrets path (never a value in settings) and the note is shown. Renderer test. **Or** MOVED: M14 (settings completeness) with the reason | DONE <pending> |

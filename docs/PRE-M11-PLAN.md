...
| X-4 | §6.7, L989 | Startup revalidation calls `validatePack` without `installedDepartmentKeys` (`revalidateInstalledPacks.ts:63`). A role whose department is in another installed pack passes install, then is marked `failed` on the next boot | Revalidation passes the other installed packs' departments, as install does. Test: install two packs with a cross-pack department reference, restart, and the pack stays `ok`. Mutation-confirmed | DONE <pending>: revalidateInstalledPacks passes installedDepartmentKeys (the same query installPack uses, excluding the pack being checked). revalidationCrossPackDepartment.test.ts scaffolds two real packs, gives the second a role in the first's department, installs both and runs the startup revalidation: the pack stays ok and available. Mutation (passing an empty list) fails it; the packs folder is green (59) |

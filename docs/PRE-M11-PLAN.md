...
| N-8 | The unconfirmed-`autonomous` → `guided` downgrade is pinned by unit tests only | One S2-style case in `policyRealEvaluator.test.ts` through the real HTTP path, asserting `ask` | DONE <pending>: S2 case in `policyRealEvaluator.test.ts` (unlisted `Bash` command, autonomous unconfirmed → permission checkpoint + hold deny; confirmed → allow, no checkpoint). The `ask` is asserted through the checkpoint, since the HTTP verdict is only allow/deny. Mutation (downgrade disabled) caught |

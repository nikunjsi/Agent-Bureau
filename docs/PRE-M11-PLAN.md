...
|---|---|---|---|---|
| T-1 | Checkpoint state machine (M8) | Property test: **no sequence of events leaves a task blocked with no pending checkpoint.** §19 calls this "the deadlock that would make the product feel broken", and M11 is what drives this machine | Generated event sequences (raise, answer, expire, auto-resolve, cancel, restart grace, message delivery) run against the real checkpoint and task code. The invariant is asserted after every step, and a mutation that introduces a stuck `blocked` task is caught | DONE 703fd99: fast-check 4.10.1 added as a devDependency (licence MIT, checked with npm view and the installed package.json). noStuckBlockedTask.property.test.ts generates up to 25-step sequences (raise with or without a safe default, blocking or not; raise permission; user answer; system resolve; expiry inside and past the grace; restart cancelling permission checkpoints) against the real functions, and checks after every step that a task blocked on a checkpoint has it pending (60 runs). A non-vacuity case proves the query reports a stuck task. Mutation (timeout answers not unblocking) caught with a counterexample after 17 runs. Out of scope and stated: non-checkpoint block reasons, and message delivery (no task state) |
| T-2 | Redactor (M6) | Property test: for **any** secret and **any** chunking of a stream containing it, the secret never appears in output. Today's tests cover two hand-picked splits | Generated secrets (values and pattern matches) and every split point / random chunkings through the real `RedactionStream`. The M3–M6 chunk-isolation mutation stays caught | DONE 1346b94: redactionStream.property.test.ts generates registered secret values and pattern-shaped secrets (sk-, gsk_, AKIA), in streams whose head is asserted longer than the 4096-character hold-back window (a first draft let fast-check pick short streams, which never exercised chunking). The real RedactionStream is fed at every split point across the secret, with random multi-chunk splits and one-character chunks around it, and the secret never appears. A third property checks non-secret text survives any chunking byte for byte. The M3-M6 chunk-isolation mutation is caught, and so is a too-short hold-back window |
| T-3 | Policy evaluator (M6) | A fuzz pass asserting **no input produces an accidental allow**, on top of the table-driven tests | Generated tool calls (names, paths, commands, including traversal, junction-shaped, mixed-case and `mcp__` shapes) through the real `evaluate()`. Nothing matched by an immutable deny is ever allowed, and nothing out-of-policy falls through to allow | DONE b69fdb1: policyNoAccidentalAllow.fuzz.test.ts runs generated calls (real and mixed-case tool names, mcp__ shapes, random names; paths over the real worktree, project, scratch, outside, a junction, system paths, with traversal and credential segments; git, destructive and ordinary commands with random whitespace) through classifyTool, extractArgs and evaluate(), over the immutable denies plus the shipped developer role's rules, 1500 runs per oracle. Oracles: deny wins, and nothing out of policy is allowed. **It found two real accidental allows at every autonomy level**: LS was not named in deny.read_outside_project (it listed any directory), and Grep, Glob and LS were not named in deny.credential_paths (a Grep inside .ssh/ would print a key). Both rules widened, with §11.3 YAML and a §0.1 row, and exemplars added (N-10's coverage test requires them). Mutations caught: each widening removed, and immutable denies ignored by the evaluator. Two oracle false positives were corrected along the way (ToolSearch takes no path; §11.3's globs cover inside .ssh/, not the directory itself) |
| T-4 | IPC (M2) | **Every** handler fuzzed with malformed payloads: none crashes, none coerces. S14 covers representative cases only | A test that iterates `methodList.ts` and sends malformed inputs to every handler through the real router, asserting a typed `VALIDATION_FAILED` and no throw. New methods are covered automatically | DONE a19ed14: ipcFuzzEveryHandler.test.ts iterates allIpcChannels() (109 methods, so new methods are covered automatically). For each, fixed hostile payloads (null, NaN, prototype pollution shapes, oversized strings, wrong types, operator objects) plus 60 fast-check anything() samples, keeping those the method's real input schema rejects, go through the real dispatchIpcCall with the real handler under a spy. Each returns VALIDATION_FAILED, nothing throws, and the handler never runs. Every method rejects at least one payload. Mutation (router skipping validation) fails the suite |
| T-5 | Migrations (M1) | Each migration applied to a fixture DB **from the previous version**. Today it is only applied to an empty DB | For each migration N, a DB built at N−1 **with representative rows** migrates to N and keeps its data (checked by row counts and a few spot values) | DONE 230bff7: migrationsKeepData.test.ts applies the real migrations one at a time through runMigrations, from 0001 to the latest. Before each step every table holds a representative row (a generic seeder reads table_xinfo, foreign keys and CHECK constraints, so new migrations are covered automatically). After each step every prior row must still exist with every column value unchanged. Mutations to real migration files caught: 0008 deleting usage rows (row count) and 0007 renaming employees (column value). Chosen over row counts plus a few spot values: it checks every value |
...
| ID | What | Done when | Status |
|---|---|---|---|
| S-1 | `permissions.hookSelfDeadlineMs` is registered but nothing reads it. `bureau-hook` takes its deadline from `BUREAU_HOOK_SELF_DEADLINE_MS` with a hardcoded 30-minute fallback. §7.10 says it is "strictly less than the registered hook timeout, **validated at startup**", and that validation doesn't exist (owner M4) | The setting feeds the hook's environment at launch, and startup validates it against the registered hook timeout, failing closed with a readable error. Tested. Or marked reserved in §16.1 with the reason | DONE d2e184c: controlChannel/hookTiming.ts resolves both settings and refuses a self-deadline that is not positive or not strictly below the registered hook timeout (maxHoldMinutes + 5 min), with a readable UserFacingError. The claude-code adapter takes the timing as an option (validated in its constructor) and puts it in BUREAU_HOOK_SELF_DEADLINE_MS and the hook registration; createClaudeCodeAdapterFromSettings(db) is the settings-reading constructor. main() validates at startup, after migrations, failing closed with an error box and exit. settings.set validates the pair before writing, so Settings can never produce a pair that stops the app starting. hookTimingFromSettings.test.ts (8 cases); mutations caught: env from defaults, validation disabled, settings.set guard removed. §F: M11 must build adapters through the factory |
| S-2 | `pty.readyDebounceMs` (global, per engine) is registered but unread. The generic-PTY adapter takes debounce from engine options (owner M3/M8) | Precedence decided (setting → per-engine override) and wired, with a test. Or marked reserved | DONE 10d20f3: marked reserved (the row's second option), after checking the code. The adapter reads the role's engine_options.ready_debounce_ms, whose schema defaults to 150, so an omitted role value can't be told apart from an explicit 150. Nothing implements a per-engine settings override (overridableBy has no reader). Precedence decided and written into §16.1's notes for whoever wires it (explicit role value, then engine override, then global), owner M14, §0.1 row. The Settings UI label for dead settings is S-5's |
| S-3 | `orchestrator.idleStopMinutes` is unread, and **no Supervisor ever idle-stops.** §7's supervisor limits list "idle-stop after `orchestrator.idleStopMinutes` (→ `off`, still assignable)"; §22.3 says an idle employee must not hold a process (owner M3) | The Supervisor stops an idle employee after the setting's interval → `off`, still assignable, with one event per state change. Tested with a FakeAdapter. Or MOVED to M11 with the reason recorded | DONE 62fefd1 (built, not moved): the Supervisor records on its monotonic clock when it entered idle. On the existing heartbeat tick it stops an employee idle for orchestrator.idleStopMinutes: stop() ends at off, with employee.stopping carrying reason idle. Exempt: the Director (mustNotStop, §8.0 keeps it warm), any employee holding a task (it still owns that task), and a setting of 0. Tests in supervisor.test.ts: stops past the limit and not before, stays assignable through a new Supervisor, and both exemptions. Mutations removing either exemption caught; the engine, messages, security, cost, control-channel, chat and checkpoints folders all green (409) |
| S-4 | Event types whose owner has passed but that nothing emits: `tool.asked`, `tool.executed`, `tool.failed` (M4/M6); `checkpoint.expired` (M8, where expiry is recorded as `auto_resolved`); `user.message_sent`, `user.checkpoint_answered`, `user.employee_paused` (deliberately not emitted per M8/M9's one-event rule); `company.created` (no production path creates a company before M13); `company.department_added` (reasoned in `installPack.ts:157`) | Each one is either emitted where it marks a real state change (invariant #3), or annotated in §5.2 the way `employee.ready` already is: documented-but-not-emitted, with the reason and owning milestone. §0.1 row added. `check:event-taxonomy` still green | DONE f103f17: every listed type checked against the code (no emitter for any of them). All annotated in §5.2 as documented-but-not-emitted, the employee.ready way. tool.asked is the permission checkpoint's checkpoint.raised; tool.executed/failed change no Bureau state (owner M14, if the Inspector wants a tool trail). checkpoint.expired is auto_resolved with appliedDefault (decided at M8). user.message_sent/checkpoint_answered/employee_paused are chat.message_persisted, checkpoint.answered and employee.parked with reason user_paused (decided at M8/M9). company.created/department_added have no producer before the wizard and addDepartment (owner M13). §0.1 row added; check:event-taxonomy green (142 types) |
| S-5 | 17 more settings nothing reads, owned by later milestones: `director.*` ×3, `intake.maxRounds`, `reporting.heartbeatMinutes`, `orchestrator.stallTimeoutS`/`maxReassignments`/`maxConcurrentEmployees`, `review.*` ×2 (M11); `floor.*` ×2 (M12); `general.sounds`, `general.keepAwake` (M13/M14); `retention.transcriptDays`/`eventTableDays` (nothing prunes; M15); `updates.channel` (M15) | Each setting's owning milestone recorded (registry metadata or a §16.1 note). **The current Settings UI does not present a dead setting as if it works**: such settings are hidden, or labelled as not yet active | DONE 5531af1: a mechanical scan found exactly 18 keys with no reader in src/: the 17 listed plus pty.readyDebounceMs (S-2); orchestrator.idleStopMinutes is read since S-3. Each carries inactiveUntil in SETTINGS_REGISTRY: director x3, intake, reporting, orchestrator x3, review x2 → M11; floor x2 → M12; general.sounds → M13; general.keepAwake and pty.readyDebounceMs → M14; retention x2 and updates.channel → M15. The Settings panel labels such a key 'Not in use yet' and disables its input. settingsNotYetActive.test.ts scans src/ both ways (unread without the marker fails, marked but read fails) and renders the field. Mutations caught: label removed, an active key marked |
| S-6 | `npm audit`: production dependencies have 1 high (`fast-uri`) and 2 moderate (`hono`, `qs`), all transitive with fixes available. Development: 1 critical (`vitest`, direct) and high `vite`, `js-yaml`, `@xmldom/xmldom`. No vulnerability check exists anywhere in the project | `npm audit fix` (without `--force`) applied, and the full sweep still green. Whatever needs a breaking upgrade is recorded with its owner (M15, alongside risk #36). A CI decision recorded on adding `npm audit --omit=dev --audit-level=high` | DONE 12c1c6b: npm audit fix --ignore-scripts (no --force; scripts skipped so the Electron-built native modules are untouched) changed only package-lock.json: fast-uri 3.1.8, hono 4.13.8, qs 6.16.0, js-yaml 4.3.2, @xmldom/xmldom 0.8.15. npm audit --omit=dev now reports 0. Remaining dev-only advisories need breaking upgrades (vitest 2→5 critical with @vitest/mocker and vite-node, vite high, esbuild 0.24→0.28 moderate) and are recorded against M15 on risk #36. CI decision: added npm audit --omit=dev --audit-level=high as a gate after install. Unit suite green after the fix (988); the full §G sweep confirms the rest |
...
|---|---|---|---|---|
| X-1 | §6.2, L835-841 | The pack layout's `templates/` (brief, plan, deliverable) and `skills/*.yaml` don't exist in any pack, and nothing reads either. Role `skills` is a bare string list. No §28 item owns them | Each is built (content in the engineering pack, validated at load, and a real consumer) **or** §6.2 annotated with its owning milestone and what M7 shipped instead, with a §0.1 row | DONE 6af51b6: claim checked (no templates/ or skills/ directory in any pack; no reader in src). Annotated in §6.2 with owners, the row's second option: templates/ → M11 (the Director writes briefs and plans and delivers; §8.3's schema stands in until then), skills/*.yaml → M14 (the second pack decides; M7 shipped skills as a string list), and assets/sprites/ (optional, trace 6.2-7) → M12. §0.1 row added |
| X-2 | §6.3, L863 | `requires.engines` ("at least one must be available") is parsed and never checked | Install/startup validation (or hire) checks the pack's `requires.engines` against the engine probe. A pack with none available is reported unavailable with a readable reason. Tested both ways | DONE e952027: revalidatePackEngines (revalidateInstalledPacks.ts) probes each validated pack's requires.engines after startup revalidation. If none is installed, or Bureau has no adapter for it, the pack is recorded failed with a readable reason ('needs one of these engines, and none of them is installed: …'), emitting company.pack_validation_failed, and isPackAvailable then refuses hiring. An indeterminate probe keeps the pack available. Wired in main() with the real claude-code probe through the shared probe cache, not awaited. packEngineRequirements.test.ts covers both ways plus the no-adapter and indeterminate cases; mutations caught (indeterminate treated as absent, check never recording). §F: per-boot ok/failed event churn on a machine without the engine |
| X-3 | §6.4, L895 | `default_hires` is validated, but nothing hires them. `company.addDepartment` is `stub('M13')`, and §28 M13 doesn't list it | Built with `addDepartment` (hires via `hireEmployee`, tested), **or** §6.4 and §28 M13 annotated with the owner, with a §0.1 row | DONE 9d3ade9: annotated (the row's second option), because building it means building M13: company.addDepartment is stub('M13') and the handler already reasons that deciding a company's departments is the wizard's act. §6.4 records that default_hires is parsed and validated but nothing hires from it, owner M13; §28 M13 item 7 now names addDepartment/removeDepartment and the hiring through hireEmployee. §0.1 row added |
| X-4 | §6.7, L989 | Startup revalidation calls `validatePack` without `installedDepartmentKeys` (`revalidateInstalledPacks.ts:63`). A role whose department is in another installed pack passes install, then is marked `failed` on the next boot | Revalidation passes the other installed packs' departments, as install does. Test: install two packs with a cross-pack department reference, restart, and the pack stays `ok`. Mutation-confirmed | DONE 364142e: revalidateInstalledPacks passes installedDepartmentKeys (the same query installPack uses, excluding the pack being checked). revalidationCrossPackDepartment.test.ts scaffolds two real packs, gives the second a role in the first's department, installs both and runs the startup revalidation: the pack stays ok and available. Mutation (passing an empty list) fails it; the packs folder is green (59) |
| X-5 | §6.7, L1000 | A pack that fails revalidation is withheld only at hire/fire. Its departments still appear in `company.listDepartments` and the floor layout, and `packs.list` returns `enabled:false` with no reason | A failed pack's departments and roles are withheld from `company.listDepartments` and the layout input (existing employees kept, per §6.7). `packs.list` output carries `last_validation_error`. Schema updated. Tested | DONE 2009bad: listDepartments gained fromAvailablePacksOnly (a department whose pack is disabled or failed is withheld; a pack-less department is kept), used by company.listDepartments and collectLayoutInputs, so a failed pack's rooms leave the floor too. PackInfoSchema gained lastValidationError, filled from the row (null for a bundled pack not installed). §6.7's keep-existing-employees half asserted: the already-hired developer survives and only new hires are refused. failedPackIsWithheld.test.ts covers all of it; three mutations caught (company listing everything, layout keeping the rooms, the reason not surfaced). check:ipc-surface green |
| X-6 | §6.8, L1028 | `rehireEmployee` keeps id and memory, but has no IPC method or production caller. A user hire into the same role creates a new employee with empty notes | A production path reaches `rehireEmployee` (an IPC method, or `company.hire` choosing rehire for an archived employee of that role, per a recorded decision), tested end to end. **Or** MOVED to M11's hire proposals with the reason | DONE c394ee9: built rather than moved. company.hire now rehires through the real rehireEmployee: with no name it brings back the most recently archived employee of that role (same id, name and notes); with a name it rehires the archived employee of that name and otherwise hires someone new. Decision recorded in §6.8 with a §0.1 row. Three cases in fireAndRehire.test.ts through the real IPC dispatch, including the memory row surviving; mutations caught (never rehiring, and a new name rehiring anyway). The company folder is green (49) |
| X-7 | §8.0, L1669 | The Director's autonomy is "fixed at `guided`, not user-configurable". `director.yaml:51-52` says this "is enforced in code", but `employees.updateSettings` sets any autonomy on any employee, the Director included | `employees.updateSettings` refuses an autonomy change for an `is_director` employee with a typed, plain-language error (budget changes still allowed). Test. Mutation-confirmed | DONE 7fb9b26: employees.updateSettings refuses an autonomy change for an is_director employee with a typed VALIDATION_FAILED and a plain sentence ('The Director's autonomy is fixed: it always asks before anything meaningful. You can still change its budget and model.'), refused before any write, so a call carrying a budget alongside writes neither and emits no event. Budget and model tier still work for the Director; an ordinary employee is unaffected. Three cases in employeeControlHandlers.test.ts; the mutation (guard disabled) fails the refusal case |
| X-8 | §9.2, L1932 / invariant #8 | `consequence: z.string().min(1)` accepts a whitespace-only consequence, and `checkpointAnatomy.test.ts:53` pins that as accepted | Validation trims before the length check (or rejects blank). The test is rewritten to assert rejection. Mutation-confirmed | DONE fa07916 |
| X-9 | §9.2, L1925/L1934 · §9.5, L1954 · §28 M8 item 2 / invariant #7 | "`default_action` is always the safe, reversible choice" and "nullable only when no reversible option exists" are unchecked. Options carry no reversibility, so a checkpoint can time out into any option, or have all-reversible options and a null default and never expire | Options state reversibility (a schema field, amended in §9.2/§5.1 with a §0.1 row). Validation rejects a `default_action` naming an irreversible option, and a null `default_action` when a reversible option exists. Tests for both refinements, mutation-confirmed. S12 still green | DONE c666a5e |
| X-10 | §9.2, L1935 | "Checkpoint creation runs a duplicate-check against answered checkpoints": `duplicateDetection.ts:70-73` checks only `decision`/`information`, so agent-raised `approval`/`blocker`/`review` checkpoints skip it. (Consulting memory, brief and workspace is §28 M11 item 14) | Every agent-raised type except `permission` is duplicate-checked, tested per type, **or** the exclusion and its reason written into §9.2 as an in-place note with a §0.1 row | DONE 697ef36 |
| X-11 | §9.4, L1945 · §28 M8 item 6 and gate | Nothing in the Core writes a `checkpoint` conversation message, and the chat card renders only from one (`MessageRow.tsx:212-224`). A raised checkpoint never appears in chat, and M8's gate "answered **from the UI**" is proven only at the IPC handler. `NEXT-VERSION` §J.5's outcome says surface 1 is built | The Core writes a `checkpoint` message (one event) when a checkpoint that doesn't wait for the Director's grouping is surfaced: at least `permission` and `blocking`. An e2e test raises a real permission checkpoint, answers it through the rendered chat card, and the held agent proceeds. §J.5 corrected | DONE 6b1fb5b |
| X-12 | §9.7, L1968-1971 | A producer should insert its message and update task state in one `BEGIN IMMEDIATE` transaction. `answerCheckpoint` unblocks the task (`:224-232`) and inserts the message (`:342-357`) as separate writes | Both writes (and any other producer's pair) run in one transaction. A kill-point test between them shows no unblocked task without its message, or the reverse. Mutation-confirmed | DONE bbae1b0 |
| X-13 | §12.1 (M10 amendment), L2372 | "`memory.reindex` hashes unconditionally" (the repair for a stamp that lies). `reindex({full:false})` is stamp-skipping, `full:true` wipes pins, and the `force` scope that hashes without wiping has no caller | Non-full `memory.reindex` uses the `force` scope. Test: a file changed with mtime and size preserved is re-indexed by `memory.reindex` and pins survive | DONE c5323c8 |
| X-14 | §12.3, L2396 · §12.5, L2416 | The memory pack's "company standards + role playbook + project decisions" clauses include only **pinned** notes. Pack-seeded standards are written unpinned, so the engineering conventions enter only through keyword search. The decision log reaches only roles whose `memory_scopes` include `project` ("every employee reads this") | Composition includes seeded company standards and the role playbook whether pinned or not, and `project/decisions.md` for every employee on a project, within the budget. Tested against the shipped engineering pack. **Or** §12.3/§12.5 annotated with what the clauses actually include, with a §0.1 row | DONE 8b47a54 |
| X-15 | §12.4, L2402 | Memory-proposal reviews are "raised at most once per phase". Reuse only covers a still-pending review, so once one is answered the next proposal in the same phase raises another | A proposal in a phase whose review was already raised attaches to the next phase's batch (or a recorded equivalent), tested with two proposals either side of an answer. **Or** the rule's pre-M11 meaning (phases exist only at M11) recorded in §12.4 and MOVED: M11 | MOVED: M11 (§28 item 15) — phases have no production writer pre-M11, so the rule cannot be made true or tested; §12.4 now states what M10 actually built |
| X-16 | §14.4, L2645 · §9.1, L1909 · §28 M9 item 7 | The Checkpoints view shows only title and context in `created_at` order. It lacks: `blocking` first, the chat card, `J`/`K`/`1`–`9`/`Enter`, answered checkpoints kept for the session, and single-keypress permission answers | The view renders `CheckpointCard`, sorts `blocking` first, and keeps the session's answered checkpoints with their decision. Keyboard `J`/`K`/`1`–`9`/`Enter` works, and a permission checkpoint is answerable with one key. An e2e test drives it by keyboard only | DONE 394bb5f |
| X-17 | §14.9, L2675 · §28 M9 item 7 | A pinned note in the memory list has an icon and a screen-reader-only label, so there's no **visible** label ("never colour alone", icon *and* label) | A visible "Pinned" label beside the icon in the list, with a renderer test asserting it | DONE 3c06d02 |
| X-18 | §14.9, L2676 | Editing an `employee/` note from the Memory view always fails: `memory.write` passes `employeeId: null` and confinement refuses employee scope without an owner | The view derives the owning employee from the note path and the handler confines to that employee's directory (still refusing traversal and junctions), **or** the view offers no Edit on `employee/` notes and §14.9 says so. Handler-level test either way | DONE cb8e72d |
...
| ID | What | Done when | Status |
|---|---|---|---|
| M3-6 #24 | No coverage tooling in the repo, only a `--no-save` install | `@vitest/coverage-v8` added as a devDependency, a `test:coverage` script, baseline recorded (not a CI gate yet). `NEXT-VERSION` §E.1 updated | DONE 8ed6764 |
| M3-6 #27 | §5.1 says `secrets_meta` holds "no values", but `storage_ref` is DPAPI ciphertext | §5.1 corrected, §0.1 row added | DONE 4d565ee |
...
| R-2 | `docs/AUDIT-M0-M2.md` §6(b) prints its status lines as one block after item 4 | Each status under its own item | DONE bfcc31d |
| R-3 | Known Issues rows whose status may no longer be true | The soak row ("Not fixed") checked against M3–M6 #15's fix (`SOAK_TIMEOUT_MS = 900_000`, which held). The two-suites-at-once row marked **accepted practice: one suite at a time**. The `genericPtyAdapter` row folded into D-3's pattern row. Every row states its real status | DONE 8df870b |
| R-4 | `NEXT-VERSION` §H.8 may have been settled by `55b4c22`, which deliberately keeps `zeroCostMode` off the cache | §H.8 marked resolved-by-decision, or left open with the reason | DONE 9ca974b |
...
| ID | What | Done when | Status |
|---|---|---|---|
| D-1 | `ProbeOptions.budgetMs` defaults to the 30 s ceiling, so a UI caller that forgets a budget hangs | `budgetMs` required; every caller passes one; typecheck enforces it | DONE 16ae6cf |
| D-2 | The shutdown sequence stops no Supervisor (none exist in production yet) | Supervisors are stopped in `runShutdownSequence` before the control channel drains, tested with a live FakeAdapter Supervisor | DONE a84d71b |
...
| D-4 | `EmployeeContext.effectiveAutonomy` (`src/shared/engine/types.ts:156`) is read by nothing. `NEXT-VERSION` §H.7 says fix this **before M11 builds the context composer** | Field removed from the interface (§H.7 option 1), or kept with a real reader. §H.7 marked resolved | DONE 6051a73 |
| D-5 | Nobody has proven that a crash **during** `reconcile()` is safe (M0–M2 report §6(c) item 7). M11 lengthens `reconcile()` | First check whether `reconcileCrashWindows.test.ts` already kills mid-reconcile and re-runs to convergence. If so: DONE, citing it. If not: one kill point inside `reconcile()`, re-run, and assert convergence with no duplicate events | DONE a7bd87b (lint follow-up 4a505d8) |
...
| E-4 | **Risk #34 (owner M3, never done): do Claude Code's terms allow orchestrated, parallel, headless use on a Pro/Max subscription?** M11 is where Bureau starts doing this at scale | **You read the current Anthropic terms and usage policy yourself** before M11 starts, and record the answer on risk row #34. A session can gather links but must not decide it | **Answered by Nikunj 2026-09-22.** E-4a (dev): Bureau-driven runs use an API key, and his Pro subscription is only for small manual checks (Consumer Terms §3 allows automated access through an API key; driving a subscription from another program is a grey area). E-4b (distributed): each user brings their own API key or cloud credential; no claude.ai login for customers without Anthropic's written approval. Sources and naming rules: `PROJECT-CHECKLIST.md` risk #34 |
...
15. **Claude Code's `--bare` becoming the default for `-p`** (E-4). Bureau
    runs `claude -p`, and `--bare` uses an API key only, never the
    subscription login. Run the adapter with `--bare` and an API key, and
    prove the policy hook (`--settings`) and Bureau's MCP server
    (`--mcp-config`) still load and the hook still fires on a tool call.
    API-key sign-in is M11's primary path; subscription login is not
    assumed to work under `-p`.
...
| S-1, 2026-09-17 | createClaudeCodeAdapterFromSettings(db) is now the only construction of a claude-code adapter that carries the user's hook timing. Nothing in production builds an adapter yet; a bare new ClaudeCodeAdapter() uses the registered defaults. The first production spawn must use the factory, or the setting is dead again for that path | M11 (plan §M11 item 2, the first production Supervisor) | No: nothing constructs adapters in production before M11 |
| X-2, 2026-09-17 | On a machine where a pack's engines are all missing, every boot records the pack ok (the synchronous revalidation) and then failed (the engine check), which is two pack_validation events per boot for one unchanged situation. Each is a real state change, but the timeline gains noise. Fix: carry an engines-unavailable failure through the synchronous pass until the engine check has run | M14 (the Packs screen and the activity timeline) | No: noise, not M11-breaking |
| X-9, 2026-09-18 | Six Core checkpoint authors state no reversibility on any option (budget exhausted, circuit breaker, supervisor budget park, dead-letter, push detected, integration merge), so none of them can ever expire. Correct under §9.5 as it stands — nothing unattended happens — but it means those six queues drain only when the user acts, and X-9 makes that visible rather than accidental | M11 (the Director surfaces and chases pending checkpoints) | No — each would need its own judgement about what a timeout may apply, which is wider than X-9's row |
| §H check 6, 2026-09-18 | Two risk-register rows had owners that had already shipped (#10 "agents report success on work that doesn't run" → M8/M5; #34 "an engine's terms prohibiting orchestrated use" → M3), and one Known Issues row had no status marker at all. Corrected during the §H sweep rather than left for the gate to trip on — the same class R-7 handled, found by the check that exists to find it | M11 (#10, §8.5.1 evaluation) and M15 (#34, licence audit) | Yes — owner corrections in PROJECT-CHECKLIST.md, the file §H reads |

...
| X-14 | §12.3, L2396 · §12.5, L2416 | The memory pack's "company standards + role playbook + project decisions" clauses include only **pinned** notes. Pack-seeded standards are written unpinned, so the engineering conventions enter only through keyword search. The decision log reaches only roles whose `memory_scopes` include `project` ("every employee reads this") | Composition includes seeded company standards and the role playbook whether pinned or not, and `project/decisions.md` for every employee on a project, within the budget. Tested against the shipped engineering pack. **Or** §12.3/§12.5 annotated with what the clauses actually include, with a §0.1 row | DONE <pending> |

...
| D-4 | `EmployeeContext.effectiveAutonomy` (`src/shared/engine/types.ts:156`) is read by nothing. `NEXT-VERSION` §H.7 says fix this **before M11 builds the context composer** | Field removed from the interface (§H.7 option 1), or kept with a real reader. §H.7 marked resolved | DONE <pending> |

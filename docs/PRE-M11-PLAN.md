...
| X-8 | §9.2, L1932 / invariant #8 | `consequence: z.string().min(1)` accepts a whitespace-only consequence, and `checkpointAnatomy.test.ts:53` pins that as accepted | Validation trims before the length check (or rejects blank). The test is rewritten to assert rejection. Mutation-confirmed | DONE <pending> |

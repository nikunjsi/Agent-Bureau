...
| T-2 | Redactor (M6) | Property test: for **any** secret and **any** chunking of a stream containing it, the secret never appears in output. Today's tests cover two hand-picked splits | Generated secrets (values and pattern matches) and every split point / random chunkings through the real `RedactionStream`. The M3–M6 chunk-isolation mutation stays caught | DONE <pending>: redactionStream.property.test.ts generates registered secret values and pattern-shaped secrets (sk-, gsk_, AKIA), in streams whose head is asserted longer than the 4096-character hold-back window (a first draft let fast-check pick short streams, which never exercised chunking). The real RedactionStream is fed at every split point across the secret, with random multi-chunk splits and one-character chunks around it, and the secret never appears. A third property checks non-secret text survives any chunking byte for byte. The M3-M6 chunk-isolation mutation is caught, and so is a too-short hold-back window |

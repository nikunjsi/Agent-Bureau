...
| `control.` | `origin_rejected`, `token_rejected`, `stale_token_deleted` (§7.9/§7.10's loopback control channel — security-relevant rejections at the transport/auth boundary, before a request ever reaches a handler; distinct from `tool.*`, which covers policy decisions on a request that was already authenticated) |

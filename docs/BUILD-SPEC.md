...
| 2026-09-24 (M11 S1-17) | §8.0.1 | States how the assembly reaches the engine: a file in the Director's state directory, appended as a system prompt, with the CLI's system-prompt snapshot off | Measured on 2.1.276's `--help`: the default snapshot would resend the first turn's system prompt on every resume, so the Director would never see its project change. The context is too long for a command line |
| 2026-09-24 (M11 S1-14) | §5.2 | `director.state_changed` added. Every Appendix A.3 transition emits exactly one event: `intake_started`, `escalated` and `replanned` name their own arrows, and every other arrow is `state_changed` | A.3's state is now persisted and moved by one function (`transitionDirectorState`), and invariant #3 needs an event for each move. Most arrows had no name in §5.2, and a type per arrow would have added sixteen types for one kind of change |
...
- On startup, `reconcile()` runs before the UI is interactive:
  - **Orphan sweep first:** for every employee row with a recorded `pid`, check whether that PID is alive **and** its start time matches `process_start_time` (PIDs are reused). If so, kill it and emit `employee.orphan_killed`. A `node-pty` process cannot be adopted across a restart — the pty master handle is gone — so adoption is never attempted. **The start-time read has three answers, not two** (M11 S1-21): alive, absent, and *could not be read*. A PID whose start time could not be read is never killed — the kill is irreversible and the PID may have been reused — and never passed over in silence either: it emits `employee.orphan_unverified` carrying the reason. Conflating the third answer with the second is how a live orphan survived a sweep with nothing in the log.
...
| `director.` | `intake_started`, `question_asked`, `report_sent`, `escalated`, `replanned`, `context_compacted`, `session_restarted`, `state_changed` (M11 S1-14: every Appendix A.3 transition emits exactly one event. `intake_started`, `escalated` and `replanned` name their own arrows; every other arrow is `state_changed`, carrying `from`, `to` and the trigger) |
...
| Tool | Arguments | Effect |
|---|---|---|
| `bureau_write_brief` | `{ brief: Brief }` (§8.3 schema) | Creates a `briefs` row (`draft`), posts a `brief` chat message, sets `awaiting_approval`. From intake it first makes §8's `intake → brief` move; a version still awaiting approval is superseded by the new one. `brief.approve` then creates the `deliverables` rows in the approval's own transaction, and `isBriefApproved` is the one answer every later step asks *(amended at M11 S2-3a; see §0.1)* |
...
| `bureau_report` | `{ kind: 'report'\|'summary'\|'question', body, payload? }` | Posts a chat message. `kind: 'question'` is how the Director asks the user anything (decision E-6): `payload.questions[]`, each with `text`, `options[]` and a `recommendation` naming one option and why. The handler refuses fewer than 2 or more than 4 during intake, a round past `intake.maxRounds`, and any question the decision log, the brief or memory already answers, handing that answer back (invariant #9) *(amended at M11 S2-2a; see §0.1)* |
| `bureau_set_project_stage` | `{ stage, reason, name? }` | Advances the lifecycle (§8); creating a project from a conversation is `stage='intake'`. `name` names a project that `intake` creates; it is required from inside another project's conversation, where the new project gets a conversation of its own *(amended at M11 S2-1b; see §0.1)*. The user's moves (approving the brief or the plan, accepting a phase) are refused |
...
| Lifetime | One process per company, and **one persistent engine session per conversation** (`conversations.director_session_id`), each resumed by `session_id` across restarts. A turn runs on its own conversation's session, so one project's transcript never reaches another's turn *(amended at M11 S2-1a; see §0.1)* |
...
**How the assembly reaches the engine (M11 S1-17).** `assembleDirectorContext` renders Appendix A.2 (`packs/operations/prompts/director.md`). Layers 2–7 fill its slots, and a dropped layer's slot says it was left out, so the Director reaches for its tools instead of assuming there is nothing. Layers 2, 4 and 6 come from one `composeMemoryPack` call, grouped by item kind. Before every Director turn, the trigger queue writes the result to `director-context.md` in the Director's state directory. The claude-code adapter passes it as `--append-system-prompt-file`, with `--system-prompt-snapshot off`: on 2.1.276 the snapshot is on by default, and it resends the first turn's system prompt on every resume, which would freeze a resumed Director's view of the project. It is a file because the context outgrows a Windows command line (32,767 characters). The CLI accepts both flags; this was measured free, with no key. Whether the text reaches the model is confirmed at the gate.
...
| 9 | **Plans that are too coarse** ("build the app" as one task) | No visibility, no recovery point | Validation: every task needs acceptance criteria; target 5–15 tasks per phase; a task exceeding its estimate by 3× triggers a re-plan. **Enforced and tested (M11 S2-4):** `bureau_write_plan` refuses a task with no acceptance criteria and a phase of more than 15 tasks ("the phase is really two"), as well as a cycle, an unknown phase or skill, and any plan before the brief is approved (`writePlan.test.ts`). The 3× re-plan trigger is §S3's |

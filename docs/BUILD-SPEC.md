...
| `company.` | `created`, `employee_hired`, `employee_fired`, `department_added`, `pack_installed`, `pack_validated`, `pack_validation_failed`, `floor_rearranged` (M7 — §6.7 validates every installed pack at startup, and recording the outcome on `packs.last_validation_status` is a real state change with nowhere else to go. Emitted only when the outcome CHANGES: a boot that finds every pack healthy writes nothing and emits nothing, so "exactly one event per state change" does not degrade into "an event whenever we looked". `pack_validation_failed` carries the readable error §6.7 requires, `severity: warn`) |

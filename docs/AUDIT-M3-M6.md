...
| 20 | MINOR | P3 | §10.3 leases | Lease exclusivity is defended by a **white-box SQL-string spy only**. `BEGIN IMMEDIATE` → `BEGIN` fails just `singleWriterAndLocking.test.ts`'s assertion that the literal string appears once. The behavioural test (25 acquirers × 30 iterations, "exactly one wins") stays green — correctly, as its own comment predicts, since better-sqlite3 is synchronous with one write connection. A change that kept the string but lost exclusivity would pass. | `src/main/db/repositories/worktrees.ts:124`; `tests/integration/singleWriterAndLocking.test.ts` | Accept as-is (documented), or add a real multi-process contention test. | S | Untouched — MINOR, deliberately not fixed this session **CLOSED at pre-M11 §C (2026-09-18): the premise was measured wrong.** The M3–M6 regression check redesigned the mutation to bite (Part 5, row 5′): inverting the lease expiry predicate — `lease_expires_at < ?` to `> ?`, a one-character change a review would pass over — is caught **behaviourally** by `leaseAcquire.test.ts` (25 concurrent acquirers, `got 25`) and by the live-lease refusal test. The original mutation (`immediate()` removed) changes no behaviour at all: the transaction body is a single conditional UPDATE, which SQLite makes atomic either way, so the spy is the only thing that could catch it and is right to. What #20 feared — keeping the string while losing exclusivity — is caught. The real gap the check found is a different one and has its own row (N-11: nothing on the production path acquires a lease), resolved by decision at pre-M11 §E-2. |
...
| 26 | MINOR | P1 | Dead data | `usage.computed_cost_usd_micros` is written on every insert and read nowhere — the column's stated purpose ("a real disagreement stays a visible, queryable fact") is unfulfilled. | `src/main/db/repositories/usage.ts:66`; `src/main/engine/supervisor.ts:646`; no readers | Add the comparison it exists for, or note it as reserved. | S | Untouched — MINOR, deliberately not fixed this session **MOVED to M14 at pre-M11 §C (2026-09-18)** — verified still true (the column has one writer, `supervisor.ts`, and no reader anywhere in `src/`). It is not a defect to fix here: the fix is a *display*, and the surface that would carry it is M14’s costs work. §28 M14 gains item 8, which names the choice — show the disagreement, or drop the column and amend §5.1. |
